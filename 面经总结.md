...
- Bresenham算法：**Bresenham算法的思想是将像素中心构造成虚拟网格线，按照直线起点到终点的顺序，计算直线与各垂直网格线的交点，然后根据误差项的符号确定该列像素中与此交点最近的像素。**
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/40afc8d3ab2b4bf889873a84af9914f5.png)

![在这里插入图片描述](https://img-blog.csdnimg.cn/2b49e9133e5a4532a94ffc82b08cc4cb.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5p-z5LiJ5Y-2,size_20,color_FFFFFF,t_70,g_se,x_16)

- 逐段的贝塞尔曲线

### 52. 点在三角形边界的处理

- 为了防止一条线段多次渲染，采用左上、或者右下规则，基本方法就是创建一个三角形的包围盒，在光栅化的时候取其左边界和上边界，而不计算右边界和下边界（左上规则，右下规则相反）

### 53. 重心坐标求解

![image-20231214173052892](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20231214173052892.png)

### 54. Z-fighting

- 描述：
  - 在场景中距离视点较远的地方更容易出现物体闪烁
- 原因：
  - 透视投影会将z值推向远平面，深度缓冲中精度有误差，远处的物体被挤压得更加接近，更容易发生z-fighting现象
- 避免：
  - 深度值更加精确，那么适当的减小近平面与远平面的绝对距离，也是可以做到的
  - 根据物体距离视点的距离，在渲染的时候，进行一个计算，避免距离视点比较远的物体相距太近
  - 提高深度缓冲区的精度
  - reverseZ（浮点数有效数字位一定，浮点数越大，小数能表示的数字位越少，精度越低）



### 55. 空间中两异面直线的距离

![image-20231214175045681](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20231214175045681.png)



### 56. 根据深度信息还原位置和法线

**所有加速方法都有一个前提，那就是得到世界空间下的线性深度，因为透视投影是非线性的，这就意味着会将z值推向远平面，这也是深度图看起来很黑或者很白的原因，需要根据透视投影矩阵还原出来相机空间下的线性深度，再根据这样一个深度信息来还原出顶点的世界空间**

- 方法一：主要是根据相似关系，深度 \* tan(fov/2) \* u（NDC空间下该像素点的高度），就可以得到其在观察空间下的y值，x值也是类似的相似三角形得到，然后加上相机在世界空间下的坐标，就可以重建出位置。对于相机空间中的一个点，从相机空间的原点、也就是相机的位置除法去与该点连线，会与近平面形成交点，这个交点的也就是我们看到的像素点，这样子可以形成一个相似三角形。
- 方法二：后处理中，可以先计算出近平面左上左下右上右下四个点 与相机构成的四个向量，然后传入顶点着色器，再像素着色器中自动插值出各个像素对应的向量，然后再×该像素的深度就可以得到在观察空间下该点的坐标，也是他相对于相机的偏移，然后加上相机在世界空间下的坐标，就可以重建出位置
- 方法三：VP逆矩阵

- 得到位置信息之后，可以找**像素上下左右四个点的位置信息，然后叉乘得到法线**

### 57. 几何阶段的输入和输出

- 输入是模型空间的顶点坐标，输出是屏幕空间的三角形顶点坐标
...
- Mipmap[i+1]中下标为(x, y)的像素的uv坐标即为(x/w, y/w)，对应的Mipmap[i]的四个像素的uv则为(2x/2w, 2y/2w)、(2x/2w, (2y+1)/2w)、((2x+1)/2w, 2y/2w)、((2x+1)/2w, (2y+1)/2w)，即：

  > Mipmap[i+1]的(u, v)对应Mipmap[i]的(u, v)、(u, v+0.5/w)、(u+0.5/w, v)、(u+0.5/w, v+0.5/w)。

- 确定boundingbox最长的轴覆盖了原始深度图的多少个像素，然后取log即可确定

### 60. 如何判断两个矩形相交

- 两个矩形左下角点横坐标的最大值小于右上角横坐标的最小值
...
- 许多非常小的纹理，一种比较好的办法是把这些纹理组合成一个比较大的纹理
- 使用压缩纹理
- 在纹理图的选择上，如果使用的颜色模式为 16 位即可达到可以接受的效果，就尽量不要使用 32 位的，也就是说尽量使用颜色模式小的纹理图

在使用方面：

-  对于大场景贴图而言，进行纹理采样时要尽量**使用 mipmap**，虽然这样相对而言会占用一些内存，但是，这样会提高纹理采样的效率，同时会得到更好的画面效果。
- 进行纹理采样时，若最近点**采样就能满足视觉效果的要求**，**就不要使用线性采样**。由于线性采样算法比最近点采样算法复杂很多，其执行效率要比最近点采样低不少。
...
- 防止画面帧数高于显示器最大刷新率时，出现屏幕撕裂的情况，对画面的帧数进行限制

### 66. LOD

- 让画面上占用像素越少的物体采用更粗糙的模型

### 67. obj格式文件和mtl格式文件

- v：顶点；vt：顶点纹理坐标；vn：顶点法线；usemtl：跟mtl文件建立对应关系；f：面
- mtl文件：定义材质属性 kd、ks、使用的纹理图等等

### 68. 透视除法的意义

- 将坐标系从裁剪空间转换到归一化的设备坐标系中

### 69. 包围球和有向包围盒

包围球：

- 分别计算所有元素顶点的x、y、z坐标的均值确定包围球的球心，再由球心与三个最大值坐标所确定的点的距离确定半径r
- 优势：对于不规则的物体冗余空间相对于AABB包围盒较小，且可以跟着对象一起旋转（AABB包围盒为了保证轴对齐不能跟着对象一起旋转）

有向包围盒：

- 不再是轴对齐的包围盒，计算相交更加困难，但是更加紧凑，冗余空间更小，但是支持旋转

### 70. 光照探针

- 全局光照探针（Global Illumination Probes）：全局光照探针用于捕捉和模拟室内或室外环境的全局光照情况。它们通常是球状或半球状的探针，用于捕捉环境中的光照强度和方向，并生成全局光照贴图。全局光照贴图可以用来在渲染中模拟全局光照效果，使虚拟场景更加真实。全局光照探针对于创建逼真的照明和阴影效果非常重要。
  - 获取该位置的全局光照信息，unity中使用了三阶球谐函数进行处理，保存各个方向的光照信息

- 反射光照探针（Reflection Probes）：反射光照探针用于捕捉并模拟物体表面的反射光照。它们通常是球状或立方体状的探针，用于记录环境中的反射信息，包括物体表面反射的光线、颜色和强度等。反射光照贴图可以用于计算物体表面的反射效果，并提供逼真的反射特性。反射光照探针对于增加物体表面的真实感和逼真度非常有用。
  - specular项，通过向6个方向渲染场景实现




### 71. IBL一般会把图像存储在cubemap中，如果在一些移动端设备上，没有提供cubemap的相关api，这个时候该怎么处理？

- 使用2D纹理：将环境贴图存储为2D纹理（Texture）而不是cubemap。这样可以在移动设备上使用常规的纹理采样和处理技术来实现IBL效果。然而，由于2D纹理无法捕捉到立体环境的所有方向，可能会丢失一些环境光照细节。
- 使用球形贴图（Spherical Texture）：将环境贴图转换为球形贴图，并将其存储为2D纹理。球形贴图可以用来模拟环境光照的球形分布。在渲染过程中，可以将反射向量映射到球形贴图上，然后从贴图中采样环境光照数据。

### 72. 如果需要实时进行卷积操作，diffuse部分怎么做可以更快，更高效？

- 球形谐波函数（Spherical Harmonics）：使用球形谐波函数来近似和压缩环境光照信息。球形谐波函数是一组基函数，可以用较少的系数来表示输入辐射度分布的近似值。通过使用球形谐波函数，可以减少对环境贴图的采样和计算，并在渲染时使用较少的计算资源。

### 73. 后处理LUT

- 相当于一种映射关系，把原来的RGB映射出新的RGB，主要采样时要不能使用插值，要使用邻近采样

### 74. OpenGL抖动阶段

- 在OpenGL渲染管线中，抖动（Dithering）是一个可选的渲染阶段，用于在颜色深度较低的情况下模拟更高的颜色精度。抖动的目的是减少颜色带来的可见条纹和明显的渐变，以提高图像的质量。位于颜色缓冲区写入之前的最后一个管线阶段，对颜色进行微小的随机调整。这些调整会添加一些噪声，并尽量使颜色在视觉上更加平滑。
...
- 所谓齐次坐标就是为矢量或者矩阵增加一个维度，2D平面使用3维向量和三维矩阵，3D空间使用4维向量和4维矩阵；额外的坐标值是任意的，可以看作缩放或者权重。
...
- 也就是出现了锯齿，出现的原因主要是将数学意义上的一个点用屏幕空间的一个有一定大小的方块代替了，在频域上可以将采样理解为频谱的搬移，采样点不够将导致频谱搬移的时候高频信息有混叠

### 77. 粒子系统和效果

- 点精灵、四边形、几何体或GPU粒子，同时做一些物理的模拟，还有就是可能需要一个对象池的概念来管理粒子的生成和回收



### 78. 后处理渲染管线

- 指的是一张已经渲染好的二维图像，还可能存在一些问题（如锯齿、亮度不足、真实度不够等等），或者想要个性化定制某种效果（比如卡通风格、素描风格等），那么可以通过后处理管线进行一系列处理，提高图像质量，使得最终效果更加逼真。
- 抗锯齿：使用不同的技术来抑制锯齿现象，如多重采样抗锯齿(MSAA)、快速近似抗锯齿(FXAA)等。
- 运动模糊：模拟物体或相机的运动造成的模糊效果。
- 环境光遮蔽：模拟光线在场景中的遮蔽效果，使得物体间的阴影更加真实。
- 色彩校正、色调映射、亮度调整、对比度调整等，用于调整最终图像的色彩和外观。



### 79. 点斜式直线如何与点法式平面求交点

![image-20231222163232989](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20231222163232989.png)

### 80. 多种光源

- 平行光：方向、环境光分量、漫反射分量、高光分量
- 点光源：位置、环境光分量、漫反射分量、高光分量、常量衰减系数、线性衰减系数、平方衰减系数
- 聚光灯：位置、方向、环境光分量、漫反射分量、高光分量、常量衰减系数、线性衰减系数、平方衰减系数、内角和外角



### 81. 光线追踪

- 在物体加载进来，转换到到世界空间的时候便针对每个模型建议自己的加速结构，也就是计算BVH包围盒，构成一个空间二叉树，每个树节点存放他的包围盒、左右节点，并在子节点中存放物体
- 建立场景时再次建立整个场景的BVH包围盒
- 然后就是渲染的部分，使用了多线程，每个线程处理一部分的像素点
- 打光线与场景求交，如果打到光源直接返回，如果打到物体，则对光源进行采样并判断可见性，可见的情况下根据蒙特卡洛积分的方法计算该点的直接光照，这是直接光照的计算，还有就是间接光照的计算，再以一定的概率去随机往场景中打一根光线，再递归的与场景中的物体求交计算shading，注意得到的结果要同时除以1/2pai和俄罗斯轮盘赌的概率，将直接光照和间接光照的结果相加并返回。
...
- Maxwell架构：全局光照
- Turing架构：引入光线追踪模块
- GPC -> TPC -> SM -> SP（CUDA核心）、Tensor、L1Cache、ShareMemory

GPU逻辑管线：

- CPU将指令推送到一个指令集缓冲供GPU读取，并将数据传输到GPU
- GPU将得到数据分发到各个GPC中处理，处理的单元还是各个sm，按照每32个线程也就是一个warp同时执行相同的指令（跟硬件有关，一套warp可以处理一种逻辑，芯片可以做的更小更快），也就是说，如果是有分支结构，并且所有一个warp中不同线程进入了不同分支，那么实际上所有分支都要执行一遍。shader变体的思路就是基于此，将运行时的多种状态转换成编译时的多种状态，提高程序执行的效率。warp调度是克服内存读取延迟的关键，比如一个warp需要读取显存中的数据，那么在读取时就可以先切换到另一个warp中执行。
- 在像素处理阶段，每32个像素线程分为一组，或者说8个2x2的像素块，这是像素着色器上的最小工作单元，这意味着**一个小的三角形即使只占了三个像素点，实际上也有可能调用12个线程来处理**，多级的mipmap不仅仅是对渲染效果的提升，更重要的作用是减少内存带宽的消耗，考虑不使用mipmap的情况，对于两个相邻的像素点，他在贴图上的对于的取样位置可能相差很大，这就降低了缓存命中率，每次做纹理采样都需要去显存而不是L1、L2缓存中读取，而mipmap可以

- 注意每个Cuda核心执行一个像素块（2x2个像素），对于**同一像素块分属于不同三角形就会分配到不同sm中处理**，因此相同面积的区域，**如果所属的三角形越多就会导致分配给sm的次数越多，消耗的渲染性能越多**
- vs和ps的数据可以直接通过L1、L2缓存进行传递
...
- 减少三角形数
- 避免使用过小的三角形（占据屏幕像素少的三角形），由于warp机制的存在会导致很多不必要的线程被开启
- 减少渲染状态的设置
- 开启mipmap降低cache miss
- 避免alpha test、alpha blend的使用，会导致early-z失效
- 减少函数调用
- 减少代数运算、减少if-else分支
- 充分利用纹理的多个通道，甚至可以将他们通过位运算合并到一个通道



### 83. 纹理压缩格式

**本质上都是纹理上的一块区域用一个数据块来进行近似**

- DXTC
  - DXT1：主要用于没有alpha信息的贴图，将4x4的像素块压缩成一个64位的数据块，这个像素块中包含两个16位RGB颜色值，代表4x4像素块颜色的极端值（可以插值出中间的两个颜色），将剩下的32位平均分配给16个像素块
  - DXT2：单纯为每个像素多记录4位的alpha信息，数据块为128位
  - DXT4：alpha也进行插值，两个8位的alpha极端值，然后每个像素3位索引值
  - ATI1：针对8位单通道数据压缩
  - ATI2：针对16位双通道进行数据压缩
  - BC6：针对半精度浮点数据
  - BC7：针对8位RGB或RGBA数据
- ETC：在移动端被广泛应用
  - ETC1：4x2的像素块进行压缩，对RGB颜色进行压缩，有亮度表，通过亮度索引（4位）和像素索引（2）位得到亮度补偿值，叠加到基础颜色上得到当前像素的颜色值
  - ETC2：长宽位2的幂次的贴图，支持alpha通道的压缩
- ASTC：数据块大小固定位128位，像素块大小从4x4到12x12不等
- PVRTC：涉及到频域的低频信息和高频信息

### 84. 顺序无关透明物体的渲染

### 85. QEM减面

- 边塌缩，两个顶点合并，顶点数减一并减少一到两个三角形

### 86. 并行LOD选择

- 将所有cluster拍平成一个数组，并行的去判断每个cluster是否需要被绘制，被绘制需要满足cluster的父节点不满足而当前节点刚好满足
...
- RenderQueue：给物体赋值，决定物体渲染顺序，2500是透明和不透明的分界点
...
### 5 Unity不同的渲染管线的区别是什么

- 默认渲染管线：这个管线只是Unity对底层渲染API(如OpenGL等)做了简单封装搞出来的，**并且当时Unity并没有去考虑这个管线的可定制性**
- SRP：可编程渲染管线
- URP：一套内置的写好的可编程渲染管线，提高了一些渲染质量



### 6. unity中的渲染优化计数

- GPU Instance：简单概括一句话就是：传递一个对象的Mesh，指定其绘制次数和材质，Unity就会为我们在GPU的`统一/常量缓冲区`开辟好必要的缓冲区，然后以我们指定的材质对Mesh进行我们指定次数的渲染，这样就可以达成一次Drawcall绘制海量对象的目的（相同材质相同物体）。
- static batch：静态合批，不能减少drawcall，只能减少setpasscall，不能移动，**占用更大的内存，最多只能支持64k个顶点**
- dynamic batch：动态合批，动态合批是专门为优化场景中共享同一材质的动态GameObject的渲染设计的。**目标是以最小的代价合并小型网格模型，减少Drawcall**。动态合批的原理也很简单，在进行场景绘制之前将所有的共享同一材质的模型的顶点信息变换到世界空间中，然后通过一次Draw call绘制多个模型，达到合批的目的。模型顶点变换的操作是由CPU完成的，所以这会带来一些CPU的性能消耗
  - 限制：900个顶点属性以下。**使用多个pass的Shader是绝对不会被合批。因为Multi-pass Shader通常会导致一个物体要连续绘制多次，并切换渲染状态。这会打破其跟其他物体进行Dynamic batching的机会。延迟渲染是无法被合批**
- SRP Batch：**并不会减少 DrawCall，减少是 DrawCall 之间的 GPU 设置的工作量**，比如有一些shader上的常量缓冲区内的数据，不同Drawcall直接使用的是一样的，那么就不需要重新设置他
...
- 首先根据根据距离相机z值得远近，将视锥体划分成一块块梯形，再根据定向光源的方向做一个正交投影，构建一个定向光源建立的空间的AABB的包围盒，以这个包围盒为新的视锥体去渲染场景中的物体的到深度信息，这样就得到了光源空间下的cascadeshadowmap。

- 根据数学推导计算出来的为了确保每一级cascade shadow map在相机空间的分辨率一致，需要将相机空间的视锥体按照距离相机的远近等比分割，但实际上一级的csm可能会很空旷，会造成资源浪费，可以跟线性分割插值一下

### 8. 阴影平坠

- 该技术旨在减少沿光源方向渲染阴影贴图时使用的光照空间范围，这可以提高阴影贴图的精度，减少阴影痤疮。
...
![image-20231219212241048](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20231219212241048.png)

### 9. JobSystem

- 用于多线程
- NativeArray：是Unity中的一种原生数组类型，用于在Unity的Job System中进行高效的数据处理。与普通的数组不同，NativeArray使用了一种更为底层的内存分配方式，可以让数据更快地在多线程之间传递和处理。同时，NativeArray还提供了安全的内存管理机制，避免了在多线程处理中出现的数据竞争和内存访问冲突的问题。NativeArray通常需要在使用完毕后手动释放内存，以避免内存泄漏。
- IJobFor



### 10. GPU Driven pipeline

- 遮挡剔除
  - 使用上一帧的深度图，将他根据相机和物体的变换投影到这一帧，作为使用，还可以利用启发式算法，先找到靠近相机的大物体，利用他们先绘制一遍深度图，在用上一帧的深度图做填充
  - two-phase的遮挡剔除，先对于这一帧的物体，用上一帧的深度图做一遍遮挡剔除得到这一帧的zbuffer，再用这个zbuffer重新做一遍遮挡剔除
- 深度图
  - 主要思路就是：对于在相机空间看过去被遮挡的物体，意味着它实际上在做阴影可见性测试的时候根本不会被采样到，那么在生成光源空间的深度图的时候就可以不用去绘制他们
- visibility buffer
  - 对于一个复杂场景，传统的defer shading**需要多次读写GBUFFER**，也需要对纹理做多次双线性、三线性采样，解决思路就是对于第一趟pass的shading，可以只存储这个可见像素对于的物体的ID（包括instanceID，triangleID等），在第二趟pass在对这个三角形做重心坐标插值求出位置颜色法线等参数
- virtual geometry
  - 根据屏幕精度建立几何绘制精度，也就是说根据几何距离相机的远近选择不同LOD的表达，尽可能减少三角形的绘制，因为一个像素最多可以承载一个三角形。
...
- **对象池是一种常用的设计模式，用于提高对象的重用性和性能。它通过预先创建一组对象并将其保存在池中，以便在需要时重复使用，而不是频繁地创建和销毁对象**。

#### 1.2 对象池实现

-  创建一个对象池类，该类包含一个用于存储对象的数据结构（例如数组、列表等），以及相关的方法。在对象池类的构造函数中，初始化一定数量的对象，并将它们添加到对象池中。
- 实现一个从对象池中获取对象的方法。该方法首先检查对象池中是否有空闲对象可用，如果有，则返回一个空闲对象；如果没有，则在需要时创建一个新的对象。
- 实现一个将对象放回对象池的方法。当对象不再需要时，可以调用此方法将对象放回对象池中，以便重复使用。
- 可选地，可以实现一些额外的功能，例如限制对象池的最大容量、定时清理不再需要的对象等。

### 2. 游戏引擎架构

![img](https://pic4.zhimg.com/80/v2-97d9bd4ffc711495106f8b3bec1c5b23_720w.webp)

- 硬件
- 操作系统
- 第三方SDK：DirectX、OpenGL、Boost、STL这样一些库和API
- 平台抽象层：对各个操作系统的线程、定时器等等做抽象与封装，主要目的是**屏蔽各个操作系统之间的差异**
- 核心层：包含内存分配管理、数学运算、文件IO等模块，主要是要高性能，避免成为性能瓶颈
- 资源管理层：主要是对各种资源如模型、纹理、音频、材质等资源进行管理
- 在上层就是各个模块系统比如渲染系统、物理系统、动画系统、脚本系统、Profiling和Debug系统等等，这些系统都相对独立
- 在上层就是编辑器UI界面
...
- 在顶点数组中维护一个动态顶点布局系统，参考stl迭代器，用来描述一组顶点属性的分布，主要包括了类型和个数（比如position是3个float，纹理坐标是2个float等），以及各个属性在数据中的偏移量，主要是为了适配OpenGL和DX11中描述顶点布局的函数。
- 动态 常量结构体构造系统，shader中有一片常量缓冲区用来传入shader计算需要的常量，如openGL中的uniform和DX11的cbuffer，他们都存在内存对齐的问题需要解决，同时，不同模型加载进来含有的贴图数量不同，如果没有这样一个系统来动态的确定常量结构体各个变量，就会存在很多ifelse的判断，代码重复度很高，比如有没有法线贴图、高光贴图等。实现的方式与上面的动态数组的方式类似，也是先构建一个layout系统描述这个常量缓冲区结构体含有什么变量类型，然后再构造一块CPU内存存放这些变量，这里就可以实现自动对齐（16个字节）的功能，有点类似运行时多态的概念。

- DX11中是16个字节的对齐，OpenGL是基于std140准则

![image-20240107161243378](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20240107161243378.png)
...
- 基于openGL和DX11的渲染器，自底向上首先是对底层的API做了一层封装，包括了vertexbuffer，indexbuffer、texture、shader、depthStencilbuffer，colorbuffer等，抽象出了openGL和DX11的共性的API。
- RenderGraph系统，将各个pass集成在一起，pass内包含了sink和source，即输入资源和输出资源，还有一些特定操作，如关闭深度写入等等
- 除此之后还包括了相机系统、日志系统、窗口系统、图形系统等。

### 4. 区别

- 采样器、深度缓冲、颜色缓冲分离、各种blend、关闭深度写入等等都抽象为可bindable的，相比之下openGL的抽象程度就没有很好，这也在我对着两套API进行统一带来了不小的挑战

- 一个用的GLSL一个用的HLSL

### 5. 坐标系

- DirectX左手系 OpenGL右手系”这个说法，这句话只适用于模型空间与视角空间
- 屏幕（视口）映射以左上为原点的坐标系，是DirectX右手系的视口坐标（原点在左上角，右为x+，下为y+，内为z+）。与之相对的OpenGL视口坐标则是左手系（原点在左下，右为x+，上为y+，内为z+）。

### 6. 亮度饱和度对比度

![img](https://pic4.zhimg.com/80/v2-82ccefce1fad944733893de73755097f_720w.webp)

### 7. DX11

顶点数据、顶点布局、顶点索引、绘制图元（三角形、点线面）、纹理、采样器、着色器、光照信息、材质信息组成的constbuffer、是否做正面、背面剔除

- context：渲染上下文设置
- device：生成可绑定对象，顶点缓冲、着色器等
- swap：定义交换缓冲区
...
-  多线程支持：引入了std::thread等多线程相关的库和特性，支持并发编程。
...
### 32. 使用extern声明static变量

- 无效



### 33. move和forward有性能上的损失吗？

- std::move是将对象的状态或者所有权从一个对象转移到另一个对象，只是转移，没有内存的搬迁或者内存拷贝。常用于延长右值的声明周期，避免无意义的拷贝

### 34. 如何给C++设计一个GC

### 35. 模板的声明和实现如何不放在头文件里
...
- 进程间通信：管道、套接字、消息队列、共享内存、信号交互
- 线程间通信：共享内存，互斥量（锁），条件变量（满足某个条件的锁）



### 37. B树和B+树

- B树：多路平衡查找树，节点存储数据，查找的最好时间复杂度可以近似O(1)，不支持范围查询
- B+树：数据只存储在叶节点中，为所有节点增加一个链指针，时间复杂度稳定在O(log n)，单个节点可以存储的信息量更多（因为不存储数据）



### 38. vector里加入10万数据，游戏中有对象的指针指向这些数据，这可能有什么问题

- 当达到vector的容量时，可能会重新寻找内存，把数据拷贝到新的内存中，这样这些指针都会失效



### 39. 多态的实现方法

- 通过父类定义虚函数，子类继承父类并重写虚函数，然后用指针或者引用的手段生成对象，调用虚函数实现多态

### 40. 怎么优化游戏中的cache访问

- 数据局部性：利用数据的局部性原理，将密集访问的数据组织在一起，以减少缓存的不命中率。例如，可以将相关的数据结构和变量进行紧凑的存储，以便在内存中更容易被缓存。
- 内存对齐
- 避免频繁的非连续内存访问
...
矩形在圆的内部、圆在矩形的内部、圆和矩形相交、圆和矩形相离四种情况，判断线段与圆的位置关系和判断圆心与矩形的位置关系，先判断两个端点是否在圆内，如果在圆外则判断离圆心的最小距离，并且还要计算垂点是否在线段上。还需要判断圆心与矩形的位置关系，方法可以有叉乘、或者夹角和为360°

### 42. 指针的好处和坏处

好处：动态分配和释放内存（做内存管理），数据的传递和共享，方便实现各种数据结构、实现多态
...
- VS自带的性能分析工具
- 写一个计时器，写一个性能分析的模块，将函数名、耗时等参数导出到json文件，然后再将其导出到网页端、像Edge和google的chrome浏览器都有相应的json可视化软件

Debug

- 根据异常可以对可能出现问题的地方定位：比如添加了一个支持相机跟随键盘按键移动旋转的功能，发现这些变换有异常，就比较容易定位到是变换矩阵出了问题。或者shading的问题像高光项感觉渲染的不对，那么就需要检查光照、法线等信息是否有异常，法线的检查可以直接将法线做一个可视化，或者直接将法向量输出到屏幕上，注意要将其转换成正值，然后用屏幕截取工具可以直接看到RGB分量。
- 总而言之就是先定位可能出现的问题点，然后再针对这些可能出现问题的地方进行逐一验证。



### 45. 函数调用

- 函数调用会新建一个栈帧，用于存放参数变量，返回地址，局部变量等数据，同时对一些寄存器里的值做保存，当函数调用结束后，栈帧会被销毁，释放内存空间，返回地址会被读取，程序会转回调用函数的位置继续执行



### 46. C++和C#的区别

- C++是编译型语言，C++是托管语言，需要运行在虚拟机上，性能不如C++，但支持反射等机制，C++可能需要自己实现一套反射
- C++通常需要程序员自己进行内存管理，提供了更底层的内存访问能力，C#有GC机制，减少了内存泄漏风险
...
- 内存分析工具，或者打开任务管理器
- operator new、delete函数，做一些日志输出打印
- 对于一些类管理的资源，确保在构造函数new的 在析构函数 delete
- 看new的个数和delete的个数是否一致
...
首先找到中间值，以中间值向第一个值依次去排序构造最大堆

交换后在重新构造

![image-20240109153031935](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20240109153031935.png)
...
- 链接器在把目标文件合并成一个可执行文件并分配好各段的加载地址后，就可以重新计算那些需要重定位的符号的具体地址
- 动态库被加载到内存之后，由动态链接器来进行
...
面试官您好，我叫曾令畅，来自华中科技大学软件学院，目前是研二在读状态。

主要的编程开发语言是C++，同时对Cuda并行计算有一定的了解。出于自己对于图形学比较感兴趣，看了Games101、202的课程，掌握包括了光栅化、光线追踪的基本原理和一些经典的图形学算法，对着色器的编写、GLGL、HLSL有一定的了解。

在项目方面是自己做了一个兼容openGL和DX11的渲染器，包括了对底层API的抽象，实现了一套renderGraph系统和一些经典的图形学算法。后续也打算基于这个架子，做一个自己的自研游戏引擎。还有一个就是目前刚刚开始做的在unity上实现的GPU驱动的渲染管线目前是做了computer shader实现的视锥剔除和遮挡剔除以及一个比较简略的cascade shadow map，还有这两天刚刚完成的参考ue源码nanite那一块在unity中实现了cluster的划分，目前正在做模型多级LOD的生成，也是打算将nanite虚拟几何这一部分在unity上做一个实现。

实习方面是从去年11月到今年3月有一段在合肥导师安排的实习，做的也是实验室的项目，一款基于Ubuntu的PET/CT的Qt桌面软件

研究方面目前是有两篇专利受理，一篇论文完成了初稿的撰写
...
- 怎么将物体对应的属性（变量名）变成一个字符串传输的界面上，编辑器上修改参数怎么传递到变量上
  - 可以建立一套反射系统，首先定义反射基类，利用C++虚表上存储的有关对象的信息，也就是RTTI，可以在运行时期获得类名等信息。
  - 在建立一个反射类，里面是一个map，map的key是类型，value是类型对应各个对象的属性

- 是否支持FBX模型的导入，模型是怎么得到的，不从网上下载有什么方法生成模型
  - 主要分为三大模块，材质、网格（包含了三角形顶点和他们连成的面，已经对应使用的材质索引）、节点（主要是用来表示模型各个部位的包含关系，里面存储了网格索引）


![image-20231225102939295](C:\Users\QZDZ\AppData\Roaming\Typora\typora-user-images\image-20231225102939295.png)

- MSAA对性能的损耗大不大，FXAA呢，两个哪个性能损耗更大，如何平衡性能与成像质量的关系，OpenGL怎么实现MSAA，启动MSAA接口对应的参数有哪些，需要怎么配置？离屏MSAA怎么在openGL中实现？
  - MSAA效果最好，FXAA性能最好，FXAA精度较低，导致画面比较糊，涂抹严重，但是对硬件要求较低
  - 跟屏幕分辨率相关，分辨率高了FXAA很慢
  - 硬件检测的？检测到硬件好自动开启？
- SSAO
- 软阴影怎么实现，除了PCSS、VSSM那一套，是否会带来性能上的损耗
  - SDF？
- 降噪算法有了解哪些
- 前向渲染和延迟渲染讲一讲
- 怎么样的shader是好的shader，性能好的
  - NVIDIA Nsight Graphics工具进行Shader Profiler
  - 着色计算位置优化：在不影响画面质量或可以略微牺牲一点画面质量的情况下，可以考虑将相关代码的位置做一些变化，以换取性能的提升（CPU、顶点着色器及片元着色器）
    - 若将这个计算任务安排到顶点着色器中，画面质量会不会有影响，若有影响，在不在可以接受的范围内。如果条件允许，则应该将相应的计算任务安排到顶点着色器中进行。因为顶点着色器的执行频率远低于片元着色器，这样做一般可以获得较为明显的性能提升
    - 判断此计算任务是对于每个顶点单独计算、结果不同，还是所有顶点共享一个相同计算结果。如果是所有顶点共享一个相同计算结果的情况，则应该将此计算任务交由CPU 执行，然后由宿主程序将计算结果作为一致变量传入顶点着色器供使用
    -  在有些情况下，对于某个复杂计算的某部分是所有顶点共享的，那么，就应该把这部分计算提取出来交由 CPU 一次完成，然后将计算结果作为一致变量传入顶点着色器供使用。对于一个 3D 模型而言，顶点少则数百上千，多则几万、几十万甚至上百万，这样做可以降低计算负载，获得显著的性能提升
  - 计算时通过代数运算简化运算：
    - 进行计算的时候，尽量使用系统自带的内置函数，因为开发人员自己开发的计算方法在大多数情况下不如系统的内建函数高效，例如，可以使用系统自带的常见函数、几何函数、向量关系函数
    - 在编写着色代码的过程中，有时候需要进行一些简单的计算，对于这些重复性较低，而且计算的内容较为简单的操作，开发过程中不应该将这种计算封装成函数，直接用代码计算即可。 这样能大大减少 GPU 函数调用与返回的消耗（这方面是目前 GPU 的弱项）
    - 如果通过调整片元颜色的 alpha 值结合混合操作就可以达到想要的效果，就要尽量少使用discard函数舍弃当前片元，因为discard 是一个非常消耗资源的操作，会大大降低渲染的 FPS
    - 由于片元着色器是在 GPU 中执行， GPU 与 CPU 在硬件结构上有很大的差异，其执行分支语句的效率非常低。故在 GPU 中需要执行分支语句时往往需要想办法用内建的函数计算出因子，再结合数学表达式来替代分支操作
- 合批有了解吗，unity的合批是怎么做的，动态合批的片元数目限制大概是多少，为什么限制动态合批的数目，为什么动态合批会影响帧率
  - 显存大小？
- 给自己的C++打几分（满分十分）
- C++多线程有了解吗，要注意什么，访问共享内存需要注意什么，除了加锁还有什么方法，原子操作底层实现是什么


### 51World一面（2023.12.21）
...
- protected
- 求点到直线的距离
- openGL中绘制一个立方体需要做哪些事情

### 51World二面（2023.12.21）
...
- 逆变换【wx、wy、wz、w】和【x、y、z、1】有什么区别
- 延迟渲染如何压缩法线纹理
- unity srp CPU端要做什么
- 并行剔除算法
- 怎么做instance合并
...
- 调式工具
  - 使用过vs自带的一个调试hlsl的工具，它可以截取想要的屏幕中的几帧图像，支持查看他的顶点着色器、像素着色器的变量值包括constbuffer和输入输出，还支持取一个像素点重新运行该帧，然后查看其在着色器中各个运算过程，还可以看绘制的模型

- FXAA用什么shader实现
- 亮度怎么计算
- Texture变体、shader变体、材质变体
- 反射
